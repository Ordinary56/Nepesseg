...
namespace WpfAppNepesseg
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            using (StreamReader sr = new(@"./Datas/kozerdeku_lakossag_2022.csv"))
            {
                sr.ReadLine()!.Skip(1);
                while (!sr.EndOfStream)
                {
                    string[] Current = sr.ReadLine()!.Split(';');
                    telepulesek.Add(new Telepules(Current[2], Current[3], Current[4], int.Parse(Current[^2].Replace(" ", "")), int.Parse(Current[^1].Replace(" ", ""))));
                }
            }
        ...
        private void cbMegyek_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            dgTelepulesek.ItemsSource = telepulesek.Where(telepules => telepules.Megye == cbMegyek.SelectedItem.ToString());
        }
    }
}
